    ...
    return (
        <div>
            <div className='rounded-2xl bg-zinc-700 h-full p-6 shadow-lg flex flex-col items-center justify-center text-center'>
                <p className='text-white text-lg md:text-xl font-medium'>Wallet Balance:{" "}
                    <span className='text-green-400 text-2xl md:text-3xl font-bold mt-1'>₹{balance}</span></p>
            ...
            >
                ...
                <form onSubmit={handleSubmit} className='flex gap-2 text-nowrap'>
                    <input type="Number" placeholder='Income Amount' value={amount} onChange={(e)=>setAmount(Number(e.target.value))} 
                    className='flex-1 border border-gray-300 rounded-xl py-3 px-2 outline-none
                    shadow-sm' />
                <div className='flex gap-3'>
                    <button
                    type='submit'
                    className='flex-1 bg-orange-500 text-white rounded-xl p-2 font-semibold shodow-md'
                    >
                        Add Balance
                    </button>
                    <button type='submit' onClick={()=>setShowModal(null)}
                    className='flex-1 bg-gray-200 rounded-xl px-3 font-semibold shadow-md'
                    >
                        Cancel
                    </button>
                </div>
